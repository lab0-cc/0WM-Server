...
let pts = [ll (-1.1) (-0.5); ll 0.5 1.1 ; ll 1.1 (-1.1)]
let p = Polygon pts
let mp = Multi_polygon [[ll 0. 0.; ll 0.2 0.; ll 0. 0.2]; pts]

(** Test for {!Zwmlib.Geo.ll_of_xy} *)
let test_ll_of_xy () =
  let { long; lat; _ } = ll_of_xy Zwmlib.Linalg.{ p_x = 12.34; p_y = 56.78 } in
...
let test_bounding_box () =
  ...
  let bb = {|{"sw":{"lat":-1.1,"lng":-1.1},"ne":{"lat":1.1,"lng":1.1}}|} in
...
let test_merge_pure_bounding_boxes () =
  let v = {|{"sw":{"lat":-1.1,"lng":-1.1},"ne":{"lat":1.1,"lng":1.1}}|} in
  let bb = [%decode.Json] ~v box in
  let v = {|{"sw":{"lat":-0.9,"lng":1.9},"ne":{"lat":1.3,"lng":3.1}}|} in
  let bb' = [%decode.Json] ~v box in
  let v = merge_pure_bounding_boxes bb bb' in
  [%encode.Json] ~v box
  |> check string "bb" {|{"sw":{"lat":-1.1,"lng":-1.1},"ne":{"lat":1.3,"lng":3.1}}|}
...
let test_obj_distance () =
  ...
  let pt = ll 0.7 (-0.5) in
  let pt' = ll 0.1 0.05 in
  let pt'' = ll 2. 2. in
