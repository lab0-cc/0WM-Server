...
type ht_op = { ht_width: ht_width [@json] [@default HT_any]; ht_chan: int [@json];
               ht_off: ht_off [@json] [@default HT_SCB] } [@@marshal]
...
type vht_width = VHT_20_40 | VHT_80_160_8080 | VHT_160_deprecated | VHT_8080_deprecated
               | VHT_reserved of int [@@marshal]
type vht_op = { vht_width: vht_width [@json] [@default VHT_80_160_8080]; vht_chan: int [@json];
                vht_sec: int [@json] } [@@marshal]
...
type he_op = { he_width: he_width [@json] [@default HE_80]; he_chan: int [@json];
               he_sec: int [@json] } [@@marshal]
...
type eht_op = { eht_width: eht_width [@json] [@default EHT_80]; eht_chan: int [@json];
                eht_sec: int [@json] } [@@marshal]
...
type f_5_ops = { f_5_ht: ht_op option [@json]; f_5_vht: vht_op option [@json];
                 f_5_eht: eht_op option [@json] } [@@marshal]
...
type cipher = AES_OCB | CCMP | CCMP_256 | CKIP | GCMP | GCMP_256 | TKIP | WEP_104 | WEP_40
            | WRAP [@@marshal]

type encryption = { protocols: protocol list [@json]; auth: auth option list [@json];
                    ciphers: cipher option list [@json] } [@@marshal]

type ap = { ssid: string option [@json]; bssid: string [@json];
